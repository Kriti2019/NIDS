...
def extract_tls_streams(pcap_file):
    ...
    for packet in packets:
        if 'IP' in packet and 'TCP' in packet:
            ...
            # Check if the packet is using TLS (check for TLS ClientHello or TLS Finished)
            if packet.haslayer(TLSClientHello) or packet.haslayer(TLSFinished):
                tls_layer = packet[TLSClientHello] if packet.haslayer(TLSClientHello) else packet[TLSFinished]
